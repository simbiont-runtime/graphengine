...
func TestLiteral(t *testing.T) {
	table := []testCase{
		{"SELECT DATE '2017-01-01' FROM MATCH ()", true, "SELECT DATE '2017-01-01' FROM MATCH ()"},
		{"SELECT TIME '12:34:56' FROM MATCH ()", true, "SELECT TIME '12:34:56' FROM MATCH ()"},
		{"SELECT TIME '12:34:56+01:00' FROM MATCH ()", true, "SELECT TIME '12:34:56+01:00' FROM MATCH ()"},
		{"SELECT TIME '12:34:56-07:00' FROM MATCH ()", true, "SELECT TIME '12:34:56-07:00' FROM MATCH ()"},
		{"SELECT TIMESTAMP '2017-01-01 12:34:56' FROM MATCH ()", true, "SELECT TIMESTAMP '2017-01-01 12:34:56' FROM MATCH ()"},
		{"SELECT TIMESTAMP '2017-01-01 12:34:56+01:00' FROM MATCH ()", true, "SELECT TIMESTAMP '2017-01-01 12:34:56+01:00' FROM MATCH ()"},
		{"SELECT TIMESTAMP '2017-01-01 12:34:56-07:00' FROM MATCH ()", true, "SELECT TIMESTAMP '2017-01-01 12:34:56-07:00' FROM MATCH ()"},
		{"SELECT INTERVAL 1 YEAR FROM MATCH ()", true, "SELECT INTERVAL 1 YEAR FROM MATCH ()"},
		{"SELECT INTERVAL 2 MONTH FROM MATCH ()", true, "SELECT INTERVAL 2 MONTH FROM MATCH ()"},
		{"SELECT INTERVAL 3 DAY FROM MATCH ()", true, "SELECT INTERVAL 3 DAY FROM MATCH ()"},
		{"SELECT INTERVAL 4 HOUR FROM MATCH ()", true, "SELECT INTERVAL 4 HOUR FROM MATCH ()"},
		{"SELECT INTERVAL 5 MINUTE FROM MATCH ()", true, "SELECT INTERVAL 5 MINUTE FROM MATCH ()"},
	}
	RunTest(t, table)
}
...
func TestModify(t *testing.T) {
 ...
 WHERE v.name = 'John'`,
			true,
			"UPDATE `v` SET (`v`.`carOwner` = TRUE),`u` SET (`u`.`weight` = 3500),`e` SET (`e`.`since` = DATE '2010-01-03') FROM MATCH (`v`:`Person`) <-[`e`:`belongs_to`]- (`u`:`Car`) WHERE `v`.`name`='John'",
		},
		{
